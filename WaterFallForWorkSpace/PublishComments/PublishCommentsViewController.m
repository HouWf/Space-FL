//
//  PublishCommentsViewController.m
...
@interface PublishCommentsViewController ()<TZImagePickerControllerDelegate,
                                            ...
                                            UINavigationControllerDelegate>
{
    ...
    CGFloat _itemWH;
    CGFloat _margin;
    CGFloat _collectionContentHeight;
}
...
@property (nonatomic, strong) PublishLocationView *bottomView;

// 允许拍照 在内部显示拍照按钮
@property (assign, nonatomic)  BOOL showTakePhotoBtnSwitch;
// 允许拍视频 在内部显示拍视频按
@property (assign, nonatomic)  BOOL showTakeVideoBtnSwitch;
// 允许拍照 在外部显示拍照按钮
@property (assign, nonatomic)  BOOL showTakePhotoBtnSwitchOut;
// 允许拍视频 在外部显示拍视频按
@property (assign, nonatomic)  BOOL showTakeVideoBtnSwitchOut;
// 照片排列按修改时间升序
@property (assign, nonatomic)  BOOL sortAscendingSwitch;
// 允许选择视频
@property (assign, nonatomic)  BOOL allowPickingVideoSwitch;
// 允许选择图片
@property (assign, nonatomic)  BOOL allowPickingImageSwitch;
// 允许选择gif
@property (assign, nonatomic)  BOOL allowPickingGifSwitch;
// 允许选择原图
@property (assign, nonatomic)  BOOL allowPickingOriginalPhotoSwitch;
// 显示一个sheet,把拍照/拍视频按钮放在外面
@property (assign, nonatomic)  BOOL showSheetSwitch;
// 照片最大可选张数，设置为1即为单选模式
@property (assign, nonatomic)  NSInteger maxCountTF;
// 每行展示照片张数
@property (assign, nonatomic)  NSInteger columnNumberTF;
// 单选模式下允许裁剪
...
@implementation PublishCommentsViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:GetImage(@"cancel_login_icon") style:UIBarButtonItemStyleDone target:self action:@selector(backClick)];
    ...
    WEAK;
    self.bottomView.publishLocationViewBlock = ^{
        STRONG;
        NSLog(@"定位");
        self.bottomView.locationStr = @"辣椒水东方丽景阿萨德六块腹肌安生；了会计分录；卡萨丁及法律；卡就是东方丽景案例是；发六块腹肌安生；了会计分录；卡萨丁及法律；卡就是东方丽景六块腹肌安生；了会计分录；卡萨丁及法律；卡就是东方丽景";
    };
}
...
- (void)defaultConfig{
    ...
    self.showTakeVideoBtnSwitch = NO;
    self.showTakeVideoBtnSwitchOut = YES;
    self.showTakePhotoBtnSwitchOut = YES;
    self.sortAscendingSwitch = YES;
    self.allowPickingVideoSwitch = YES;
    ...
    self.showSelectedIndexSwitch = NO;
}
...
- (void)viewDidLayoutSubviews {
    ...
    self.textView.frame = CGRectMake(15, 15, Main_Screen_Width-30, 120);
    [self.textView jk_addPlaceHolder:@"这一刻的想法…"];

    _margin = 4;
    CGFloat lefM = 15;
    _itemWH = (self.view.tz_width - 2 * _margin - 4 - 2 * lefM ) / 4 - _margin;
    _layout.itemSize = CGSizeMake(_itemWH, _itemWH);
    _layout.minimumInteritemSpacing = _margin;
    _layout.minimumLineSpacing = _margin;
    [self.collectionView setCollectionViewLayout:_layout];
    CGFloat collectionViewY = CGRectGetMaxY(self.textView.frame) + 15;
    self.collectionView.frame = CGRectMake(0, collectionViewY, self.view.tz_width, self.view.tz_height - collectionViewY);
    CGFloat bottomViewY = CGRectGetMinY(self.collectionView.frame) + 15;
    self.bottomView.frame = CGRectMake(0, bottomViewY + _itemWH + 2*_margin, Main_Screen_Width, 50);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    PublishCommentCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"PublishCommentCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath{
    NSInteger newContentHeight = collectionView.contentSize.height;
    if  (newContentHeight != _collectionContentHeight) {
        [UIView animateWithDuration:0.2 animations:^{
            self -> _collectionContentHeight = newContentHeight;
            CGFloat textViewOrignY = CGRectGetMaxY(self.textView.frame) + 15;
            self.bottomView.frame = CGRectMake(0, textViewOrignY + self -> _collectionContentHeight + 26, Main_Screen_Width, 50);
        }];
    }
}
...
#pragma mark - 五类个性化设置，这些参数都可以不传，此时会走默认设置
    ...
    [imagePickerVc.navigationBar setTintColor:UIColor.blackColor];
...
#if 0
    // 如果未选择图片/视频，则相册包含video/camera。
    if (_selectedPhotos.count == 0 ) {
        self.showTakeVideoBtnSwitchOut = YES;
    }
    else if (_selectedPhotos.count > 1){
        // 如果选择的内容大于一，标示选择的是图片
        self.showTakeVideoBtnSwitchOut = NO;
    }
    else{
        // 判断是否包含视频
        PHAsset *asset = _selectedAssets[1];
        BOOL isVideo = NO;
        isVideo = asset.mediaType == PHAssetMediaTypeVideo;
    }
#else
    // 如果未选择图片/视频，则相册包含video/camera。
    self.showTakeVideoBtnSwitchOut = _selectedPhotos.count == 0;
#endif
    UIImagePickerControllerSourceType sourceType = UIImagePickerControllerSourceTypeCamera;
    if ([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeCamera]) {
        self.imagePickerVc.sourceType = sourceType;
        NSMutableArray *mediaTypes = [NSMutableArray array];
        if (self.showTakeVideoBtnSwitchOut) {
            [mediaTypes addObject:(NSString *)kUTTypeMovie];
        }
        if (self.showTakePhotoBtnSwitchOut) {
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[PublishCommentCell class] forCellWithReuseIdentifier:@"PublishCommentCell"];
    }
    return _collectionView;
}

- (PublishLocationView *)bottomView{
    if(!_bottomView){
        
        _bottomView = [[PublishLocationView alloc] init];
